...
int main(void){
  ...
  while(1){          // Follows the nine steps list above
    //Ready signal goes high
		SetReady();
    //Wait for AS to fall (touch SW1 switch)
		WaitForASLow();
    //Ready signal goes low
		ClearReady();
    //Wait 10ms (debounces the switch)
		Delay1ms(10);
    //Wait for AS to rise (release SW1)
		WaitForASHigh();
    //Wait 250ms (simulates the time between atrial and ventricular contraction)
		Delay1ms(250);
    //VT signal goes high
		SetVT();
    //Wait 250ms
...
// write this function
	while(1) {
		input = GPIO_PORTF_DATA_R & (0x00000010);
		if (input == 0x00) {
			break;
		}
	}
